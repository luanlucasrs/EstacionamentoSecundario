...
public class Estacionamento {
	...
	public int menu() {
		...
		// Alocar bot√µes
		Object[] funcoes = { "Iniciar serviÁo", };
		int funcao = JOptionPane.showOptionDialog(null, "Bem-vindos ao serviÁo EstacioneX", "O que deseja fazer?",
				JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, funcoes, funcoes[0]);

		// Consultar Placa
		if (funcao == 0) {
			consultarPlaca(null); // My: /eu n√£o achei esse m√©todo no seu codigo aqui, mas adicionar dentro dele
									// o m√©todo BuscaPlaca
			 ...
			 * registrarEntradaVeiculo() Caso saida registrarSaidaVeiculo() 2.2)Caso n√£o:
			 * Cadastrar veiculo novo?(s/n) Caso sim cadastrarVeiculo() Caso n√£o Encerra
			 * 
			 */
		 ...
		 * Passo-a-passo dessa fun√ß√£o
		 * 
		 * 1) Dados Veiculo 2) Ele √© mensalista? (s/n)
		 * 
		 ...
		 * n√£o Deseja registrar entrada? (s/n) Caso sim registrarEntradaVeiculo() Caso
		 * nao sair
		 *
		 */
	...
	/**
	 * Fun√ß√£o respons√°vel pelo funcionamento do menu at√© que a interface seja
	 * fechada
	 */
	...
	public Veiculo consultarPlaca(String placa) {
		 ...
		 * rotina deve ser alterada para testar se realmente existe e n√£o por uma
		...
		if (valorDoBotao == 0) {
			...
			if (buscaPlaca(placa2) != null) { // Se placa existir
				...
				Object[] funcoes = { "Entrada", "SaÌda" };
				int tipoRegistro = JOptionPane.showOptionDialog(null, "Deseja registrar uma entrada ou saÌdaa?",
				...
				if (testaCadastro == 0) {// Se deseja fazer o cadastro
					try {
						...
						JOptionPane.showMessageDialog(null, "Dados do VeÌculo est„o incompletos.");
					}
	...
	public boolean cadastrarVeiculo() throws DadosVeiculosIncompletosException {
		 ...
		 * decis√£o se existe ou nao CNH Precisa checar nos arquivos txt se existe ou
		 * nao CNH
		 * 
		 */
		...
		if (valorDoBotao == 0) {
			...
			valorDoBotao = JOptionPane.showConfirmDialog(null, myPanel, "Cadastro de VeÌulo",
					JOptionPane.OK_CANCEL_OPTION);
			...
			while (sair != 0) {
				if (valorDoBotao == 2 || valorDoBotao == -1) {
					...
					//valorDoBotao = JOptionPane.showConfirmDialog(null, myPanel, "Cadastro de VeÌculo",
						//	JOptionPane.OK_CANCEL_OPTION);
			...
			if (valorDoBotao == 0) {
				...
				if (mensal) { /// SIM VAI SER MENSALISTA
							...
							if (valorPainel == 0) {
								...
								if (buscaCNH(checaCnh) != null) { // Se existir algo
									...
									if (checaRegistro == 0) { // se sim desja
										...
										String message1 = "VocÍ sera direcionado para Registro de Entrada";
										...
										String message1 = "Cadastro de VeÌculo feito com sucesso.";
					...
					if (checaRegistro == 0) {
						...
						String message = "Cadastro de VeÌculo feito com sucesso, vocÍ sera direcionado para Registro de Entrada";
						...
						String message = "Cadastro de VeÌculo feito com sucesso.";
	...
	public boolean cadastrarCliente() throws DadosPessoaisIncompletoException {
		...
		if (valorDoBotao == 0) {
			...
			myPanel.add(new JLabel("EndereÁo: "));
	...
	public boolean registrarSaidaVeiculo(Veiculo veiculo) {
		...
		if (veiculoMensalista != null) {
			JOptionPane.showMessageDialog(null, "Mensalista >>> SaÌda liberada");
			return true;
		}

		String message = "Digite a hora da saÌda no formato dd/MM/yyyy hh:mm";
